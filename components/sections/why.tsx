...
function BenefitCard({ icon: Icon, title, desc }: Benefit) {
  return (
    <motion.div
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] p-6 backdrop-blur transition-colors hover:border-brand-red-500/40"
    >
      <motion.div
        className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-xl bg-brand-red-500/10 text-brand-red-300 ring-1 ring-inset ring-brand-red-500/20"
      ...
      <span className="pointer-events-none absolute -bottom-6 -right-6 h-24 w-24 rounded-full bg-brand-red-500/0 blur-2xl transition-colors duration-500 group-hover:bg-brand-red-500/10" />
    </motion.div>
  );
}
